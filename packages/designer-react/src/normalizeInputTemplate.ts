...
import type {
  DesignerTemplateInput,
  DesignerTemplateKind,
} from "./types";
...
export function normalizeInputTemplate(
  ...
  throw new Error(
    "[hiprint-re/designer-react] Invalid template or cannot detect template kind.",
  );
}
...
function resolveTemplateKind(
  ...
  throw new Error(
    "[hiprint-re/designer-react] Cannot detect template kind. Please set templateKind explicitly.",
  );
}
